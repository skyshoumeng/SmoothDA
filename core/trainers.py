...
class SemanticKITTITrainer(Trainer):
    ...
    def _run_step(self, feed_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if outputs.requires_grad:
            ...
            self.summary.add_scalar('learning rate', self.optimizer.state_dict()['param_groups'][0]['lr'])
    ...
    def train(self,
        ...
        try:
            ...
            while self.epoch_num < self.num_epochs:
                ...
                self.after_epoch()
                logger.info('Current learning rate: {}'.format(self.optimizer.state_dict()['param_groups'][0]['lr']))
                ...
                self.iou = np.asarray(callbacks[0].callbacks.callbacks[0].ious)
                logger.info(self.iou)
